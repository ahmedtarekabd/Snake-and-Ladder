...
public:
	...
	void RemoveObjectFromCell(const CellPosition& pos, bool deleteGameObject = true); // Removes the GameObject of the Cell of the passed "position"
	...
	Ladder* HasLadder(const CellPosition& cellPosition) const;		// Checks if pGameObject is a Ladder Then returns it if a ladder or returns NULL if not ladder
	Snake* HasSnake(const CellPosition& cellPosition) const;		// Checks if pGameObject is a Snake Then returns it if a snake or returns NULL if not snake
	Card* HasCard(const CellPosition& cellPosition) const;			// Checks if pGameObject is a Card Then returns it if a card or returns NULL if not card

	bool startOverlapsEnd(const CellPosition& startPos) const;

	bool isOverlapping(GameObject* newObject) const;

	bool anyPlayerOwnThisCard(int cardNumber);

	void giveExtraRollDice();

	void ResetPlayerPosition(const CellPosition& cellPosition);

	void ResetOwnerships();

	Player* GetNextPlayer(const CellPosition& position);

	Player* Grid::GetPlayerLeastAmount();

	Cell* getCellFromNumber(int cellNumber);

	void clearAll();

	// ========= Save the Grid =========

	void SaveAll(ofstream& OutFile, int type) const;
